...
from schemas.film_schema import FilmCreateSchema, FilmSchema  # ← add FilmSchema

film_bp = Blueprint("films", __name__)  # url_prefix is set when you register the bp

# schemas: one for input validation, one for output
...
def list_films():
    ...
    return {"data": read_many_schema.dump(films)}, 200  # ← use read schema
...
def get_film(film_id):
    ...
    return read_schema.dump(f), 200  # ← use read schema
...
def create_film():
    ...
    db.session.add(f)
    db.session.commit()
    return read_schema.dump(f), 201  # ← return consistent shape
